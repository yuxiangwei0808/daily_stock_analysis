...
- [新功能] 交易计划回测（`scripts/backtest_trade_plans.py`，说明见 `docs/strategy-backtest.md`）：用实盘参数回放交易机会与突破提醒的确定性规则，含标普 500 历史成分重建、ETF 对照、同方向随机入场对照、组合模拟、期权定价变体与随机游走对照。
- [改进] 精简功能：移除 Trade Desk 每 15 分钟的主动期权发现（两天 73 次模型调用、72 次“等待”、0 次提醒，已由定时交易机会及其期权跟进取代）；盘中监控、突破提醒与持仓提醒合并为每分钟一次的共享 OpenD 快照，移除 15 分钟“快速波动”提醒，大幅波动提醒对持仓 3% 起、其他自选 5% 起。
- [新功能] `MARKET_REVIEW_TIMES`（仅在指定定时时点执行大盘复盘）与 `BRIEF_CHANGES_ONLY_TIMES`（指定时点的推送简报只列出结论变化的股票），减少 Discord 消息量；默认不改变原有行为。
- [改进] 自选全为美股时跳过 AkShare 名称预热；本地 CLI（Claude Code / Codex）调用计入用量记录（无 token 数，记录调用次数与模型）。
- [修复] 定时运行完成后由子进程自行标记完成，避免崩溃重启后重复推送；模型面板主结论同步为护栏后的最终结论，持有与观望视为一致；第二意见等待最多 180 秒；免费新闻全部失败时不缓存，短代码（A、ON、F 等）需明确提及才算相关；盘后无常规收盘价时简报标注“盘后”；首页自选行情失败时保留上次报价，分组获取失败会自动重试。
- [改进] 交易机会：为自选/持仓保留一半复核名额，剔除过度延伸的形态，报告倾向阈值改为 65/35，消息显示首个目标的盈亏比与“风险”行（盈亏比低于 1 的机会不推送），扫描股票显示全名；异常 K 线或模型返回格式错误只跳过单只股票。
- [改进] 突破提醒：需连续两次检查确认、10:30 前需 2 倍量能、同一股票同方向一周内只提醒一次，同指数/同标的基金（SPY/VOO、SOXL/SOXS、TSLL/TSLA 等）每天只提醒一次。
- [改进] 券商持仓：按持仓方向（股票正负、期权到期收益形态）给出措辞；已到期仍列示的期权标记为已到期且不触发规则；到期提醒注明各腿价内/价外；提醒备注中的金额与数量在 Discord 中打码；财报查询改为后台进行；手机端持仓以卡片显示；首次同步前显示提示。
- [修复] 行情快照中单个无效代码不再导致整批报价失败；Trade Desk 事件查询改为按时间与类型过滤。
- [修复] 收盘后（16:10）简报与大盘复盘把盘后涨跌当作当日涨跌：个股改用当日常规时段收盘价与涨跌幅；美股时段扫描在盘后以常规时段计算涨跌家数、板块 ETF 与异动股，盘后异动单列。
- [修复] 定时分析运行中服务重启会丢失本轮：开始后 60 分钟内重启且未完成的定时运行会自动重新开始（最多 2 次；若旧进程的分析子进程仍在运行则不重复启动）。
- [改进] 每次运行的汇总报告另存 `reports/report_日期_时分.md`，单股临时分析不再覆盖当天定时运行的报告。
- [修复] 美股无资金流数据时，趋势（均线排列/信号）已确认的买入/卖出不再因靠近支撑/压力被降级为“洗盘观察/震荡”。
- [修复] 交易机会：被截断的杠杆/反向 ETF 名称（如 “Direxion Daily Semiconductor Be”）不再被建议做空或买期权，短期国债 ETF 等不再误判；仅定时运行触发扫描，重启不重复扫描或重复推送。
- [修复] Discord 长消息按段落分片发送，重试只补发失败分片；盘中大涨/大跌回落时不再按较低档位重复提醒；突破提醒需超过关键位 0.15 ATR 并沿用交易机会的止损/目标；主动发现在“等待”结论后 4 小时内不再重复评估同一股票；关闭服务时事件流不再报错。
- [修复] 券商持仓提醒：重新启用或修改已触发提醒后可再次推送；新增提醒若当前已满足会提示；长期权按成本百分比止盈；买价为 0 时不再用过期成交价估值；同到期日换仓后提醒独立；已到期仍列示的期权不再提醒；提前收盘日的“收盘前一小时”按实际收盘时间；文字规则识别避免 close/profit、百分比与价格混淆；模型识别不再接收金额与股数。
- [新功能] 每日组合摘要：每个交易日 16:15（美东）推送一条 Discord 消息并显示在「券商持仓」页（账户当日涨跌、股票/期权/现金占比、相当于 SPY 的市场敞口及对冲来源、最大持仓与当日异动、杠杆/反向 ETF 占比、集中度、相关性对冲/同涨同跌、5 个交易日内到期与 7 天内财报、提醒状态），仅含百分比。
- [新功能] 券商持仓（只读，`TRADE_DESK_BROKER_ACCOUNT`）：通过 OpenD 读取 moomoo 账户持仓，Trade Desk 新增「券商持仓」页（期权按标的+到期日合并为价差、中间价估值、盈亏与最大收益占比）；内置短线期权提醒（到期前 2/1 个交易日、到期日开盘与收盘前一小时、卖出腿价内指派风险、止盈 75%/止损 −50%、到期前财报）与股票破位/财报提醒；可按持仓添加自定义提醒（标的价格、距到期天数、盈亏百分比，支持文字描述识别）；交易机会与突破提醒注明持仓并调整建议措辞；Discord 仅显示百分比。
...
- [修复] 美股按价格结构校准买卖结论时，提示语不再引用“主力资金流入/流出”，改为基于支撑、压力与突破的表述。

- [修复] `--serve`/`--serve-only`/定时+Web 模式按 Ctrl+C 退出时，此前不会通知 uvicorn 退出，应用 lifespan 关闭流程（停止运行时调度器及其分析子进程、释放 Trade Desk 租约）不执行，导致正在进行的定时分析成为孤儿进程继续推送；现退出时先请求 uvicorn 退出并等待，优雅关闭上限 5 秒。
- [改进] 盘中行情监控降噪：开盘后 15 分钟不发“15 分钟快速波动”提醒；杠杆 ETF（名称含 3X/UltraPro/2X/Ultra）的档位按杠杆倍数放大；每只股票每天最多 3 次快速波动提醒；只引用标题中包含代码或公司名的新闻。

- [改进] Trade Desk 到期损益图聚焦在行权价与盈亏平衡点附近（按分段线性插值截取），不再显示 0 到约 2 倍股价的全区间，窄价差不再被压成一条竖线。

- [改进] Trade Desk 所选对比的顶部直接显示 AI 结论（等待/值得比较）与解释（长文可展开），不再只在页面底部的研究版本卡片中出现；候选方案的警告不再显示报价时间等数据源诊断代码（如 `order_book_time_unreported_observed_on_live_subscription`）。

- [修复] 选股（screening）配置加载在设置了 `ENV_FILE` 时仍会额外读取工作目录与项目根目录的 `.env` 并写入进程环境；现与主配置一致，显式 `ENV_FILE` 为唯一配置文件。
- [测试] 测试不再读取开发者真实 `.env`：conftest 为每个测试提供临时 `DATABASE_PATH`/`ENV_FILE`，设置 `LITELLM_MODE=PRODUCTION`（避免 LiteLLM 导入时 `load_dotenv()`），并在收集阶段结束后撤销测试模块导入时加载的环境变量。

- [修复] 美股无资金流数据源（status=not_supported）时不再把所有“买入”结论降级为“持有观察”并覆盖一句话结论，改为按价格结构（支撑/阻力/突破）校准；A 股等支持资金流的市场保持原有降级逻辑。
- [修复] 收盘后（盘后时段）生成的美股简报此前把盘后价与“相对今日收盘”的盘后涨跌当作当日涨跌显示；现在行情记录当日常规收盘价与当日涨跌幅（`regular_close`/`regular_change_pct`），简报显示为“收盘价 (当日涨跌) · 盘后 价格 (盘后涨跌)”。

- [修复] 首页自选实时行情此前因 camelcase-keys 把股票代码键转为小写（AAPL→aapl）而无法匹配、行内不显示价格；现只转换行情字段并保留代码键。首页自选栏在大屏加宽（lg 320px / xl 384px），整体布局最大宽度放宽至 1920px。

- [修复] 大盘复盘的“主要指数平均涨跌幅”与指数评分不再计入 VIX 等波动率指数（其与股指反向变动，会使市场信号方向失真，例如三大指数微跌时平均值因 VIX +3.3% 显示为 +0.74%）。

- [修复] Google News 免费搜索在多关键词查询（如风险排查“公司名 risk insider selling lawsuit litigation”）无结果时，逐步去掉末尾关键词重试（最多 2 次），避免多数个股风险维度为空；定时分析子进程补齐日志初始化，运行日志写入 `logs/stock_analysis_scheduled_<date>.log`。

- [改进] Web 页面在宽屏下不再居中留白：去掉页面内容区（AppPage、首页、选股页）的居中最大宽度限制，内容紧贴侧边栏展开，整体宽度仍由外层布局限制。

- [新功能] 盘中行情监控（`MARKET_PULSE_ENABLED`）：常规交易时段内每 60 秒用 OpenD 批量快照检查自选美股，当日涨跌越过档位（默认 3/5/8%，每档每天一次）或 15 分钟内波动超过 2% 时推送 Discord；每 10 分钟轮询 Google News，由常规模型（或关键词规则）筛选重大新闻推送。默认关闭。

- [新功能] 首页自选显示实时行情：`GET /api/v1/stocks/watchlist/quotes` 用 moomoo OpenD 单次批量快照返回美股最新价、涨跌幅与盘前/盘后价格（缓存 10 秒），首页每 15 秒刷新（页面隐藏时暂停），价格变动时短暂高亮；自选行改为紧凑行情样式，删除按钮悬停显示，列表顶部显示当前交易时段与行情时间。OpenD 不可用时不显示价格，其余功能不变。

- [新功能] 首页自选按 moomoo 自定义分组显示（分组标签页，含“未分组”），数据经 OpenD 读取并缓存 5 分钟；新增 `GET /api/v1/stocks/watchlist/groups`，OpenD 不可用时返回 `available=false`，首页按原平铺列表显示。
- [修复] Trade Desk 在模型给出建议后重新定价遇到报价过期时，不再丢弃整段解释，改为标记为 stale；yfinance 时区缓存改为进程内存，修复长时间运行服务的文件描述符泄漏。

- [新功能] Trade Desk 支持评估用户自己的期权计划：请求可带 `plan_legs`（表单中每行一条腿，如 `buy 1 call 230 2026-10-16`），系统按这些精确合约的实时报价定价并作为“Your plan”与其他方案对比；无法定价时给出 `plan_error`。新增 `FREE_NEWS_SOURCES`（`google_news`/`yahoo_finance`/`finnhub`）：Google News RSS 作为无需 key 的新闻搜索兜底用于个股分析，Trade Desk 在用户发起对比时实时拉取新闻作为带日期的证据。

- [新功能] 模型分层与多模型意见：`GENERATION_BACKEND` 负责定时报告、大盘复盘和 Trade Desk 自动扫描（自动扫描改为无工具的低成本对比审阅）；新增 `TARGETED_GENERATION_BACKEND`、`SECOND_OPINION_BACKENDS`、`CLAUDE_CODE_CLI_MODEL`，用户主动发起的个股分析与 Trade Desk 手动对比改用指定主模型，并由其他模型在同一数据上给出独立结论，报告与 Trade Desk 页面显示“模型意见”及分歧提示。未配置时行为不变。

- [修复] 指数行情在 Yahoo 日线缺失前一交易日时把当日收盘当作昨收，涨跌幅恒为 0.00%（大盘复盘据此给出错误判断）；现取 5 日数据并在最新一根为当日时使用 Yahoo 报告的前收盘价，同时忽略收盘为空的最新日线。英文大盘复盘使用英文美股指数名称。

- [改进] 推送简报（REPORT_TYPE=brief）增加价格、涨跌幅与关键价位（仅显示与现价相符的价位），数据来源行压缩为一行；设为 brief 时 Web 发起的分析推送也使用简报。修复：美股英文报告成交量/成交额显示为“万股/元”、低于 1 美元的价格显示为 0.00 或长浮点、推送文本在单词中间截断（改为按词截断并加省略号）。股票搜索索引新增 53 只常用美股 ETF（SPY、QQQ 等），输入 SPY 不再只能选到 SPYR；新增 `generate_index_from_csv.py --us-etf-only`。

- [改进] Trade Desk 主动发现（市场扫描、Codex 分析与机会推送）仅在美股常规交易时段运行，盘前盘后不再产生模型调用。

- [修复] 应用日志根级别为 DEBUG 时 yfinance 会关闭多线程下载，导致美股时段扫描/选股在服务进程中退化为逐只下载并超时（市场发现一直无数据）；现将 yfinance 日志降为 WARNING。美股扫描在 Yahoo 日线缺失前一交易日时，用该交易日收盘前 10 分钟内的最后一根 5 分钟常规时段 K 线作为基准价。Trade Desk：只有不利方向的入场成本变化才判定过期，模型未选中的候选直接按当前报价更新；过期对比提供“重新运行”。

- [修复] Trade Desk 实盘第二轮：模型调用超过 30 秒或查看建议后再建计划时，按所选合约重新定价而非一律判为过期；冷启动标的（首次请求）不再因链加载超时或按快照开始时间计算报价年龄而丢失合约；期权链只请求实际使用的到期日；向模型提供日线与 15 分钟 K 线（未完成 K 线标注 partial），并过滤与候选无关的数据源诊断；计算候选默认证据置信度为低。服务启动端口探测与 uvicorn 一致使用 SO_REUSEADDR，重启时不再被 TIME_WAIT 连接阻塞。

- [修复] Trade Desk 实盘联调：期权新鲜度改用订单簿（快照时间为最后成交时间，未成交合约不再被误判过期；订阅中但无服务器时间的订单簿按观察时间计）；期权不再占用 QUOTE 订阅额度；过期判断按整体入场成本相对仓位规模，而非单腿一分钱跳动；ETF 当日 16:15 到期合约可用于日内，波段仍用后续到期；信用价差卖出价外行权价。

- [改进] Trade Desk 支持 OpenD 协议 RSA 加密（`TRADE_DESK_OPEND_RSA_KEY_FILE`），避免共享主机上的其他用户使用已登录的行情会话。

- [修复] Trade Desk 第二轮复核：未配置 OpenD 时正确禁用实盘请求；日志标注回放事件；到期模拟持仓可按标的价格结算；追问资金识别“5000.”/“$5k”；小幅价格变动不再误判建议过期（按所选合约重新定价）；崩溃重启后恢复中断任务；到期持仓不再逐小时断流告警；主动发现跳过非美股代码且单一代码失败不中断；波动观点默认仅做多波动；铁鹰短腿行权价不重合；回放模拟成交单独统计；计划可暂停/归档；Discord 消息禁止 @提及；行情时钟偏差容忍 2 秒；取消不再覆盖已完成结果。美股监控：昨日盘前/盘后价格不再冒充当前时段涨跌；Longbridge 与美股指数报价补充时段信息；过期报价拦截仅作用于美股（恢复 A 股/港股原行为）并统一美股新鲜度阈值；无板块数据时不再生成美股板块段落；提前收盘日盘后截至 17:00；告警时间按本地时间记录。

- [修复] Trade Desk 复核：修复前端按 ID 取触发价/快照/追问参数失效（camelCase 改写十六进制 ID）、已持股备兑计划未成交即显示持仓且模拟平仓卖出自有股票、过期未成交计划持续告警、盘外逐小时断流通知、成交按字符串排序；OpenD 适配器按订阅类型与本连接校验订阅、退订失败不再泄漏、IV 为 0 不再崩溃、标的需有效买卖价才算已验证、读取期权未平仓量、补充行情登录/权限诊断与 LEAPS 交易日历；Trade Desk 入口仅对美股显示，SSE 新连接不再回放全部历史。

- [修复] Trade Desk 追问保留最新比较参数，核对行权/指派后的完整股票交割，优先订阅持仓合约，并修复通知中的建议和持仓跳转。

- [新功能] Trade Desk：美股期权策略对比、本地 Codex 追问、模拟成交与手工实盘记录、独立行情监控及 Discord 通知；行情未验证时明确降级，支持带标识的合成回放。

- [修复] 美股盘中报价覆盖日线时同步推进前一交易日基准，统一涨跌额、涨跌幅和 LLM 上下文；历史记录、通知及模板将时段报价与带日期的常规日线分开展示，避免混用盘前盘后价格和历史成交量。

- [修复] Yahoo 基本面请求正确转换 BRK.B 等美股类别股代码，并避免把 HKD 等美股代码误当作港股前缀。

- [修复] 选股策略加载期间禁用市场切换与运行，避免迟到的策略响应把美股请求配成仅支持 A 股的策略。

- [修复] 美股扫描全部行情缺失或过期时保留覆盖数量与数据限制说明；跨数据源补充不再把常规时段量比、换手率、振幅填入盘前盘后报价。

- [修复] Yahoo 类别股报价保留 BRK.B 等应用代码及美股市场身份，恢复盘前/盘中/盘后价格与新鲜度校验，并统一日线兜底入口；补充回归测试与报告分时粒度说明。

- [修复] 美股保存报告、通知及 Web 展示报价时段、真实时间和过期提示，缺失/零时段涨跌幅不再回填日线涨跌幅；无时间或时段的兜底报价不能触发价格告警；选股输出统一 BRK.B 等类别股代码，Yahoo 请求按需转换；跨开收盘扫描共用同一时间。

- [修复] 美股 Yahoo 日线在确认常规收盘后纳入当日 K 线（含提前收盘），自选分析与选股日线补充共用日期边界；盘前盘后成交量缺失/为零的价格异动单列为流动性未验证，并保留原有选股成交额门槛与缺失数据提示。

- [修复] 美股监控保留指数常规日线日期并在报告/Web 标注；缺失有效价格不能仅凭时间戳标记新鲜；已有可用扫描时，生成超时、非零退出或空输出保留明确标注的确定性报告与失败诊断。

- [修复] 美股报告日线数据复用时段/新鲜度校验；盘外涨跌幅拒绝日期或时间不匹配的基准收盘价，选股结果及历史保留并展示行情时间、时段与基准价。

- [修复] 美股报价按盘前/盘中/盘后选择价格与真实时间，过期、缺失时间及盘外报价不能改写常规日 K；Stooq 日线兜底不再触发实时价格告警。
- [新功能] 美股五分钟分时段选股及涨跌策略，Web 支持 US 市场；大盘复盘增加股票池广度、行业 ETF 代理表现、自选外异动与数据覆盖说明。
- [修复] Reports 刷新同时重试当前报告正文，修复正文加载失败后仅刷新列表无法恢复的问题。
- [文档] 新增美股分时段监控配置、数据口径及回滚说明，提供纽约时间多时点调度示例。

- [新功能] 新增报告中心 `/reports`，按已保存的市场时段阅读盘前、盘中、盘后报告与大盘复盘，展示生成时间、分析上下文时间和日历推算的最近完整交易日；沿用已有历史 API 与报告阅读器，不触发新分析。
