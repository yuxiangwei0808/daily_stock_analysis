...
- Model tiers (optional): `GENERATION_BACKEND` serves scheduled stock reports, market review and Trade Desk automatic scans (scans use a tool-free review of the calculated comparisons). Analyses a user explicitly requests (Web/API/bot stock analysis, Trade Desk manual comparisons and follow-ups) switch to `TARGETED_GENERATION_BACKEND` when it is set, and each backend in `SECOND_OPINION_BACKENDS` (comma-separated; the primary is skipped) gives an independent verdict on the same data in parallel. Reports and the Trade Desk page show a model panel and flag split decisions. Second opinions are advisory; a failing model is shown as unavailable and never blocks the primary analysis. `CLAUDE_CODE_CLI_MODEL` sets `--model` and `CLAUDE_CODE_CLI_EFFORT` sets `--effort` (low/medium/high/xhigh/max) for `claude_code_cli`; user-requested analyses (including second opinions) use `TARGETED_CLAUDE_CODE_CLI_MODEL` / `TARGETED_CLAUDE_CODE_CLI_EFFORT` when set (e.g. Sonnet for routine runs, Opus high for second opinions); `codex_cli` takes its model and reasoning effort from `CODEX_HOME/config.toml`, so a dedicated `CODEX_HOME` keeps them separate from your global Codex config. Example: `GENERATION_BACKEND=litellm` (e.g. `openai/google/gemini-3.8-flash` on OpenRouter) + `TARGETED_GENERATION_BACKEND=codex_cli` + `SECOND_OPINION_BACKENDS=litellm,claude_code_cli`.
