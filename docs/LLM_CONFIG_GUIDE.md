...
- 模型分层（可选）：`GENERATION_BACKEND` 用于定时个股报告、大盘复盘和 Trade Desk 自动扫描（自动扫描使用无工具的对比审阅）。用户主动发起的分析（Web/API/机器人个股分析、Trade Desk 手动对比与追问）在设置 `TARGETED_GENERATION_BACKEND` 后改用该 backend，并由 `SECOND_OPINION_BACKENDS`（逗号分隔，自动排除主 backend）在同一份数据上并行给出独立结论；报告与 Trade Desk 页面显示“模型意见”及是否分歧。第二意见仅供参考，失败时标记为不可用，不影响主分析。`CLAUDE_CODE_CLI_MODEL` 可为 `claude_code_cli` 指定 `--model`，`CLAUDE_CODE_CLI_EFFORT` 指定 `--effort`（low/medium/high/xhigh/max）；用户主动发起的分析（含第二意见）改用 `TARGETED_CLAUDE_CODE_CLI_MODEL` / `TARGETED_CLAUDE_CODE_CLI_EFFORT`（留空沿用前两项），例如常规分析用 Sonnet、第二意见用 Opus high；`codex_cli` 的模型与推理强度来自 `CODEX_HOME/config.toml`，可用独立 `CODEX_HOME` 与全局 Codex 配置隔离。示例：`GENERATION_BACKEND=litellm`（如 OpenRouter 上的 `openai/google/gemini-3.8-flash`）+ `TARGETED_GENERATION_BACKEND=codex_cli` + `SECOND_OPINION_BACKENDS=litellm,claude_code_cli`。
...
- 运行时选择：`GENERATION_BACKEND`、`TARGETED_GENERATION_BACKEND`、`SECOND_OPINION_BACKENDS`、`CLAUDE_CODE_CLI_MODEL`、`GENERATION_FALLBACK_BACKEND`、`GENERATION_BACKEND_TIMEOUT_SECONDS`、`GENERATION_BACKEND_MAX_OUTPUT_BYTES`、`GENERATION_BACKEND_MAX_CONCURRENCY`、`LOCAL_CLI_BACKEND_MAX_CONCURRENCY`、`AGENT_GENERATION_BACKEND`、`LLM_CHANNELS`、`LITELLM_MODEL`、`LITELLM_FALLBACK_MODELS`、`AGENT_LITELLM_MODEL`、`VISION_MODEL`、`VISION_PROVIDER_PRIORITY`、`LLM_TEMPERATURE`、`LLM_USAGE_HMAC_SECRET`、`LLM_USAGE_HMAC_KEY_VERSION`、`LLM_PROMPT_CACHE_TELEMETRY_ENABLED`、`LLM_PROMPT_CACHE_HINTS_ENABLED`、`LLM_PROMPT_CACHE_DIAGNOSTICS_LEVEL`
