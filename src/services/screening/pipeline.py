...
def _df_to_picks(df: pd.DataFrame) -> list[Pick]:
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        picks.append(Pick(
            ...
            quote_session=_safe_text(row.get("quote_session")) or None,
            provider_timestamp=_safe_text(row.get("provider_timestamp")) or None,
            reference_price=_safe_float(row.get("reference_price")),
            is_stale=_safe_bool(row.get("is_stale")),
