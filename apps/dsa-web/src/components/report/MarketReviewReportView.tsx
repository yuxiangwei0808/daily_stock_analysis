...
const MARKET_REVIEW_TEXT: Record<ReportLanguage, {
  ...
  en: {
    ...
    change: 'Change',
    highLow: 'High/Low',
    regularSessionBar: 'Regular-session daily bar',
                        ...
                        {marketData.indices.map((index) => (
                          <tr key={index.code || index.name}>
                            <td className="px-2 py-2 font-medium text-foreground">
                              {index.name}
                              {index.dailyBarDate && <span className="mt-1 block text-xs font-normal text-muted-text">
                                {marketReviewText.regularSessionBar}: {index.dailyBarDate}
                              </span>}
                            </td>
